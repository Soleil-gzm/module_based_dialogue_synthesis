...
class RandomService:
    ...
    def __init__(self, seed: Optional[int] = None):
        self._random = random.Random(seed)      # 创建独立的 Python 随机生成器，避免全局污染，支持可重复随机
        if seed is not None:
            np.random.seed(seed)  # 同步设置 numpy 的全局随机种子，确保 np_choice 等方法也可重复
        self._seed = seed
    ...
    def shuffle(self, seq: List[Any]) -> None:      # 当前未使用。可用于随机化候选顺序，增加多样性。
        """打乱列表"""
