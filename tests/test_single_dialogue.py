...
def test_single_path_and_dialogue():
    ...
    cases, prompts = load_cases(
        cases_dir, rng
    )  # 注意：需要修改 load_cases 以接收 rng，或者先不用 rng
    case = cases[0]
    prompt = prompts[0]
    print(
        f"使用案例: {case.get('客户姓名', '未知')}, 逾期天数: {case.get('逾期天数', '未知')}\n"
    )
    ...
    while True:
        ...
        if current == "身份确认":
            ...
            print(
                f"规则: current='告知' → 排除['身份确认','转告']，候选集 = {candidates}"
            )
        elif current == "信息核实":
            candidates = [
            ...
            print(
                f"规则: current='信息核实' → 排除['告知','身份确认','三方','转告']，候选集 = {candidates}"
            )
            ...
            print(
                f"规则: current在B集({current}) → 候选集 = B集"
                + (f" + 已选A集({selected_a})" if selected_a else "")
                + f" = {candidates}"
            )
        else:
            candidates = modules[:]
        ...
        if next_node == "已还款" and current not in ["告知", "信息核实"]:
            print(
                f"违反约束: {next_node} 只能从'告知'或'信息核实'进入，当前是 {current}，重新选择"
            )
            continue
    ...
    for idx, node in enumerate(path):
        ...
        print(
            f"  选中行 uid={row['uid']}, conditions={row.get('conditions(条件)', '')}"
        )
        ...
        inserted = False
        if (
            node in builder.insert_nodes
            and len(ancestors) == 0
            and len(descendant_chain) == 0
        ):
