...
def load_sheets(excel_path):
    ...
    for sheet in MODULES:
        ...
        mask = df["conditions(条件)"].apply(
            lambda x: isinstance(x, str) and "S1|S2" in x
        )
...
def generate_dialogue(path, df_dict, case, prompt_text):
    ...
    # 添加 system 消息
    sys_content = (
        "你是一个洋钱罐的催收专员，请根据客户的情况，使用合适的话术与客户进行沟通，争取让客户承诺还款。\n"
        + prompt_text
    )
    messages.append({"role": "system", "content": sys_content})
    ...
    for node in path:
        ...
        if not valid_rows:
            logger.debug(
                f"模块 {node}, repeat={repeat}: 无候选行（条件不匹配或无数据）"
            )
            continue
        row = random.choice(valid_rows)
        ...
        logger.debug(
            f"选中行 -> 模块: {node}, repeat: {repeat}, uid: {row['uid']}, conditions: {row.get('conditions(条件)', '')}"
        )
        logger.debug(
            f"案例信息: 姓名={case.get('姓名')}, 性别={case.get('性别')}, 逾期笔数={case.get('逾期笔数')}"
        )
        assistant_full = str(row["assistant(专员)"])
        logger.debug(
            f"助理话术预览: {assistant_full[:80]}{'...' if len(assistant_full)>80 else ''}"
        )
        # 校验性别与话术中称呼的一致性
        if "先生" in assistant_full and case.get("性别") == "女士":
            logger.warning(
                f"性别可能错误: 女性案例但话术包含'先生'，uid={row['uid']}, 姓名={case.get('姓名')}, 话术片段: {assistant_full[:100]}"
            )
        if "女士" in assistant_full and case.get("性别") == "先生":
            logger.warning(
                f"性别可能错误: 男性案例但话术包含'女士'，uid={row['uid']}, 姓名={case.get('姓名')}, 话术片段: {assistant_full[:100]}"
            )
...
def main():
    ...
    if not cache_valid:
        ...
        print(
            f"路径生成完成，共 {len(all_paths)} 条，已保存至 {PATHS_CACHE}（种子: {RANDOM_SEED}）"
        )
