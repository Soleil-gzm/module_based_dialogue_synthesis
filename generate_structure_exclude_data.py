...
def generate_tree(
    root_dir, max_depth=2, current_depth=0, prefix="", is_last_root=False
):
    """
    递归生成目录树字符串
    ...
    for i, item in enumerate(items):
        ...
        if item.is_dir() and current_depth + 1 < max_depth:
            ...
            sub_lines = generate_tree(
                item, max_depth, current_depth + 1, sub_prefix, False
            )
...
def main():
    ...
    # 所以手动处理根目录下的第一级
    root_items = sorted(
        project_root.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower())
    )
    tree_lines = []
    for i, item in enumerate(root_items):
        ...
        if item.is_dir():
            ...
            if item.name in DATA_DIRS:
                ...
                sub_prefix = "    " if is_last else "│   "
                sub_lines = generate_tree(
                    item, max_depth=2, current_depth=1, prefix=sub_prefix
                )
