...
if __name__ == "__main__":
    ...
    # ---- 输出目录（自动创建）----
    SYSTEM_DIR = "my_case/cases3_system"
    REPLACE_DIR = "my_case/cases3_replace"
    ...
    for combo in combos:
        ...
        for _ in range(n):
            ...
            prompt_system = load_and_format_prompt2(
                "case_gr_Suning-new/prompt_template_for_system1-new.txt", data
            )
            with open(f"{SYSTEM_DIR}/case_{case_idx+1}.txt",
            ...
            prompt_replace = load_and_format_prompt3(
                "case_gr_Suning-new/prompt_template_for_backbone_replace.txt", data_copy
            )
            with open(f"{REPLACE_DIR}/case_{case_idx+1}.txt",
                      "w", encoding="utf-8") as f:
                f.write(prompt_replace)

            case_idx += 1

    print(f"完成，共生成 {case_idx} 条 case")
