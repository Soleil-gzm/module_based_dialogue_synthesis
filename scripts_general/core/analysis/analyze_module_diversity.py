...
def analyze_module_diversity(
    trace_file: str, target_module: str, top_conditions: int = 15
):
    ...
    print(f"共 {len(all_conditions)} 个条件")
    print("-" * 80)
    print(
    ...
    # ===== 图表数据：取 Top N =====
    top_n = min(top_conditions, len(all_diversity_scores))
    top_diversity_scores = all_diversity_scores[:top_n]
    top_condition_names = [d["condition"] for d in top_diversity_scores]

    print(
        f"\n📈 图表仅显示前 {len(top_condition_names)} 个高频条件，完整数据请查看 TXT 报告。"
    )

    if not top_condition_names:
        print("⚠️ 没有条件可以绘图")
        return

    # ===== 2. 热力图 =====
    all_uids = Counter()
    for d in top_diversity_scores:
        for uid, cnt in d["uid_counts"].items():
            all_uids[uid] += cnt
    ...
    fig, ax = plt.subplots(
        figsize=(max(10, len(top_uids) * 0.5), max(6, len(top_condition_names) * 0.4))
    )
    ...
    ax.set_title(
        f'模块 "{target_module}" 条件 × UID 选择热力图\n(前 {len(top_condition_names)} 个高频条件)',
        fontsize=13,
    )
    plt.tight_layout()
    ...
    ratios = [d["max_ratio"] for d in top_diversity_scores]
    unique_counts = [d["unique_uids"] for d in top_diversity_scores]
    colors = plt.cm.RdYlGn_r([d["diversity"] for d in top_diversity_scores])
    bars = ax.barh(
        top_condition_names, ratios, color=colors, edgecolor="black", alpha=0.85
    )
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 80 + "\n")
        f.write(f"共 {len(all_diversity_scores)} 个条件\n")
        f.write(
            f"{'条件':<30} {'总次数':>8} {'不同UID数':>10} {'最大UID占比':>12} {'多样性评分':>10}\n"
        )
        f.write("-" * 80 + "\n")
        for d in all_diversity_scores:
            f.write(
                f"{d['condition']:<30} {d['total']:>8} {d['unique_uids']:>10} {d['max_ratio']:>11.2%} {d['diversity']:>9.2f}\n"
            )
        f.write("\n" + "=" * 80 + "\n")
        f.write("解读指南:\n")
        f.write(
        ...
        f.write(
            f"\n📈 图表仅显示前 {len(top_condition_names)} 个高频条件，完整数据见本报告。\n"
        )
    ...
    for d in all_diversity_scores:
        if d["max_ratio"] > 0.5:
            print(
                f"  🔴 {d['condition']}: 最大UID占比 {d['max_ratio']:.1%} (仅 {d['unique_uids']} 个不同UID)"
            )
            alert_count += 1
    if alert_count == 0:
...
def main():
    ...
    parser.add_argument(
        "--module", "-m", required=True, help="要分析的模块名称 (如: 信息问题)"
    )
    parser.add_argument(
        "--top", "-n", type=int, default=15, help="显示前 N 个条件 (默认: 15)"
    )
